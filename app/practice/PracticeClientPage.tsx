...
function KeypadButton({
  ...
  return (
    <button
      ...
      disabled={disabled}
      className={[
        "rounded-2xl px-4 py-3 text-lg font-bold shadow-sm transition sm:text-xl",
        disabled
...
export default function PracticeClientPage() {
  ...
  const renderQuestionBody = useCallback(() => {
    if (!currentQuestion) {
      return (
        <div className="rounded-3xl bg-amber-50 px-5 py-6 text-center text-amber-900">
          問題を読み込めませんでした。
        </div>
      );
    }
    ...
    switch (currentQuestion.layout) {
      case "sum":
        return (
          <div className="mx-auto max-w-sm rounded-3xl bg-slate-50 p-4 shadow-inner ring-1 ring-slate-200">
            <div className="mb-3 text-center text-xs font-semibold text-slate-500 sm:text-sm">
              {config.typeLabel}
            </div>
            <div className="space-y-1.5 font-mono text-right text-2xl font-bold tracking-wide text-slate-900 sm:text-3xl">
      ...
      case "multiply":
        return (
          <div className="mx-auto max-w-sm rounded-3xl bg-slate-50 p-5 text-center shadow-inner ring-1 ring-slate-200">
            <div className="mb-3 text-xs font-semibold text-slate-500 sm:text-sm">
              かけ算
            </div>
            <div className="font-mono text-2xl font-bold tracking-wide text-slate-900 sm:text-3xl">
      ...
      default:
        return (
          <div className="rounded-3xl bg-red-50 px-5 py-6 text-center text-red-900">
            未対応の問題形式です。
          </div>
        );
    }
  ...
  if (finished) {
    return (
      <main className="min-h-screen bg-gradient-to-b from-orange-50 to-white px-4 py-5 text-slate-800 sm:px-6 sm:py-7">
          ...
          <section className="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-slate-200 sm:p-7">
            ...
            <div className="mt-5 flex flex-col gap-3 sm:flex-row">
              <button
                ...
                className="inline-flex items-center justify-center rounded-2xl bg-orange-500 px-5 py-3.5 text-base font-bold text-white transition hover:bg-orange-600"
              >
                同じ設定で再挑戦
              </button>
              ...
              <Link
                href="/settings"
                className="inline-flex items-center justify-center rounded-2xl border border-slate-200 px-5 py-3.5 text-base font-semibold text-slate-700 transition hover:bg-slate-50"
              >
                設定に戻る
              </Link>
                ...
                <>
                  ...
                  <div className="mt-5 rounded-2xl bg-slate-50 p-4">
                    ...
                    <input
                      ...
                      disabled={checked}
                      placeholder="数字を入力"
                      className="mt-3 w-full rounded-2xl border border-slate-300 px-4 py-3 text-center text-2xl font-bold tracking-widest outline-none transition focus:border-orange-500 focus:ring-4 focus:ring-orange-100 disabled:bg-slate-100 disabled:text-slate-500 sm:text-3xl"
                    />
                    ...
                    {checked && currentQuestion && (
                      ...
                      >
                        {isCorrect
                          ? "正解です。"
                          : `不正解です。正しい答えは ${currentQuestion.answer} です。`}
                      </div>
                    )}
                  </div>
                  ...
                  <div className="mt-5 flex flex-col gap-3 sm:flex-row">
                    <button
                      ...
                      className={[
                        "inline-flex items-center justify-center rounded-2xl px-5 py-3.5 text-base font-bold transition",
                    ...
                    <button
                      ...
                      onClick={clearInput}
                      disabled={checked}
                      className="inline-flex items-center justify-center rounded-2xl border border-slate-200 px-5 py-3.5 text-base font-semibold text-slate-700 transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:bg-slate-100 disabled:text-slate-400"
                    >
                      入力クリア
                    </button>
